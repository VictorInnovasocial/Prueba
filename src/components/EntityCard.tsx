    ...
    <article className="card group flex flex-col h-full">
      <div className="p-5 pb-3 flex-1">
        ...
        <div className="flex items-start gap-3 mb-4">
          {/* Avatar */}
          <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-primary-100 to-primary-200 flex items-center justify-center flex-shrink-0 text-2xl leading-none shadow-sm">
            {entity.countryFlag}
          </div>

          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <h3 className="text-base font-semibold text-neutral-800 group-hover:text-primary-700 transition-colors leading-snug">
                {entity.name}
              </h3>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {entity.expertise.slice(0, 3).map((skill) => (
            <span
              key={skill}
              className="text-xs px-2.5 py-1 bg-neutral-100 text-neutral-600 rounded-full font-medium"
            >
              {skill}
            </span>
          ))}
          {entity.expertise.length > 3 && (
            <span className="text-xs px-2.5 py-1 bg-neutral-100 text-neutral-400 rounded-full">
              +{entity.expertise.length - 3}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-4 text-xs text-neutral-400">
          <span className="flex items-center gap-1">
            ...
            {entity.projectsCount} proyectos
          </span>
          <span className="flex items-center gap-1">
            <Users className="w-3.5 h-3.5" />
            {entity.teamSize} personas
          </span>
